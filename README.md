...
CaloPlan 是一款基于 Vue 3 构建的个人饮食管理工具，帮助用户记录每日饮食、追踪营养摄入、管理体重数据，并通过 AI 获取个性化的饮食推荐与建议。

## 功能特性

- **仪表盘** — 布局清晰的双栏界面，左侧对话交互，右侧数据看板
- **今日卡片** — 查看今日体重、距目标进展、连续打卡天数、预计减重趋势及问候卡片
- **营养追踪** — 记录今日营养摄入（热量、蛋白质、碳水、脂肪），并与历史平均值对比
- **饮食记录** — 添加/查看今日餐食明细，支持食物条目管理
- **AI 推荐** — 基于用户数据智能推荐饮食方案
- **AI 拍照识别** — 预设拍照上传入口，支持后续 AI 识别食物并自动填入营养数据
- **智能聊天** — 对话式交互，支持饮食咨询与建议
- **打卡系统** — 支持每日打卡，记录连续天数
- **数据持久化** — 使用 Pinia + localStorage 持久化用户数据
- **多语言支持** — 支持中文和英文界面切换
- **Mock 数据** — 内置模拟数据，方便开发与演示
...
│   │   ├── DashBoard.vue      # 仪表盘容器
│   │   ├── Dialog/            # 弹窗组件
│   │   │   ├── DialogAddMeal.vue  # 添加餐食弹窗（含营养表单、AI拍照识别入口）
│   │   │   └── DialogCheckIn.vue  # 打卡弹窗
│   │   ├── MealRecord/        # 饮食记录
│   │   │   ├── MealRecord.vue
│   │   │   ├── MealRecordItem.vue
│   │   │   ├── TodayMealRecord.vue
...
使用 Pinia + `pinia-plugin-persistedstate` 插件，将 `mealRecords`、`mealEntries`、用户配置等数据自动持久化至 `localStorage`。
